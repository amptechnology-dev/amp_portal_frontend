...
function Page() {
  ...
  const fetchContactDetails = async () => {
    try {
      const response = await axios.get(
        `${process.env.NEXT_PUBLIC_BACKLINK}/dashboard/contact_us/list`,
  ...
  const handleUpdate = async () => {
    try {
      ...
      await axios.put(
        `${process.env.NEXT_PUBLIC_BACKLINK}/dashboard/contact_us/update/${id}`,
  ...
  //     try {
  //       await axios.delete(`http://localhost:8443/api/dashboard/contact_us/delete/${id}`,
  //       {withCredentials:true}
  //     );
  //       fetchContactDetails();
  //     } catch (error) {
  ...
  const handleDelete = async (id) => {
    swal({
      ...
      if (willDelete) {
        try {
          await axios.delete(
            `${process.env.NEXT_PUBLIC_BACKLINK}/dashboard/contact_us/delete/${id}`,
            { withCredentials: true }
          );
          fetchContactDetails();
